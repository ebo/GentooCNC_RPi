...
src_prepare() {
	eautoreconf
	epatch "${FILESDIR}/remove_ldconfig-2.6.8.patch"
...
src_configure() {
	...
	use !modbus && myconf="${myconf} --without-libmodbus"
	use modbus && myconf="${myconf} --with-libmodbus"
	use !gtk && myconf="${myconf} --disable-gtk"
	use rt && myconf="${myconf} --with-rt-preempt"
	use simulator && myconf="${myconf} --enable-simulator"
